...
import {deriveMyUrashimaSource} from '../scripts/build-my-urashima.ts';
import {prepareStoryDates} from '../scripts/refresh-story-artifacts.ts';
import {
  formatLocalDate,
  planStoryDate,
  readStoryDate,
  storyDateMetadata,
} from '../scripts/story-date.ts';
...
/** ローカル日付だけを返す時計。UTC 変換が使われていないことを保証するためのテスト用ダブル。 */
function localClock(year: number, monthIndex: number, day: number): Date {
  return {
    getDate: () => day,
    getFullYear: () => year,
    getMonth: () => monthIndex,
