...
import type {AssetManifest} from '../scripts/story-config.ts';
...
test('keeps the deprecated Text Asset fixture buildable with 3.1 and 3.2 headers', async () => {
  const [fixture, assetManifest] = await Promise.all([
    ...
    readFile(path.join(sampleDirectory, 'assets.lock.json'), 'utf8').then(
      (contents) => JSON.parse(contents) as AssetManifest,
    ),
  ]);
  ...
  try {
    for (const version of ['3.1', '3.2']) {
      ...
      const project = JSON.parse(
        strFromU8(unzipSync(sb3)['project.json'] as Uint8Array),
      ) as ScratchProject;
      const stage = project.targets.find((target) => target.isStage);
      assert.ok(stage, 'the rebuilt SB3 must contain a stage target');
