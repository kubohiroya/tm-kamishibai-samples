...
test("builds a deterministic published SB3 and tutorial distribution archives", async () => {
  ...
  const [config, webLock, publicSurfaces] = await Promise.all([
    readFile(path.join(storyDirectory, "dsl4-build.config.json"), "utf8").then(
      JSON.parse,
    ),
    readFile(
      path.join(storyDirectory, "dsl4-web-artifacts.lock.json"),
      "utf8",
    ).then(JSON.parse),
    readFile(path.join(storyDirectory, "public-surfaces.json"), "utf8").then(
      JSON.parse,
    ),
  ]);
  ...
  assert.equal(
    build.artifactLock.publication.reason,
  ...
  assert.equal(
    config.work.thumbnail.licenseHref,
    "stories/tutorial/LICENSES.md",
  );
  ...
  assert.deepEqual(build.artifactLock.outputs.sb3.targetNames, [
    "Stage",
    "Student",
  ]);
  assert.deepEqual(
  ...
  assert.deepEqual(
    Object.keys(
      unzipSync(new Uint8Array(build.archives.additionKit.bytes)),
    ).sort(),
...
test("turns the starter into the final four-scene story with visible pose feedback", async () => {
  const [starter, addition, complete] = await Promise.all([
    ...
    readFile(
      path.join(storyDirectory, "addition-kit/add-pose-scene.yml.txt"),
      "utf8",
    ).then((source) => parse(source, { uniqueKeys: true })),
  ...
  assert.equal(
    starter.scenes.earthquake[2]["Student.say"].text,
    "なにがおきたの？",
  );
  ...
  assert.equal(
    complete.scenes.protect.actions[2]["Student.pose"].steps[0].pose,
    "頭を守る",
  );
...
test("reuses the kneeling hands-on-head pose model with an explicit safety label", async () => {
  const sourceModelDirectory = path.join(
    storyDirectory,
    "../urashima/pose-models/6and7",
  );
  ...
  const [
    ...
    readFile(path.join(sourceModelDirectory, "metadata.json"), "utf8").then(
      JSON.parse,
    ),
    readFile(path.join(tutorialModelDirectory, "metadata.json"), "utf8").then(
      JSON.parse,
    ),
  ...
  assert.deepEqual(tutorialMetadata.labels, [
    "open1",
    "open2",
    "open3",
    "頭を守る",
...
test("provides one intentional CLI diagnostic with an exact starter fix", async () => {
  const [diagnosticExercise, starter] = await Promise.all([
    readFile(
      path.join(
        storyDirectory,
        "addition-kit/intentional-diagnostic.kamishibai.yaml.txt",
      ),
      "utf8",
    ),
  ...
  assert.equal(
    diagnosticExercise.replace("Student.sya:", "Student.say:"),
    starter,
  );

  const kamishibaiRoot = path.resolve(
    process.env.TMPOSE_KAMISHIBAI_DSL4_ROOT ??
      path.join(projectRoot, "../tmpose-kamishibai"),
  );
...
test("renders every fixed tutorial download on the published detail page", async () => {
  ...
  for (const href of [
    ...
    assert(
      ...
      `Tutorial detail page is missing ${href}.`,
    );
  }
  assert(html.includes(manifest.sourceIdentity));
  assert(html.includes(manifest.artifacts.web.sha256));
  assert(html.includes('<time datetime="2026-08-15">2026年8月15日</time>'));
  assert(html.includes("21.8 MB（21,848,205 bytes）"));
