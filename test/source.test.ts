...
import type {Dsl4Story} from '../scripts/dsl4.ts';
import type {ScratchProject} from '../scripts/scratch.ts';
...
} from '../scripts/build-packaged-web.ts';
...
function readSb3Project(contents: Uint8Array): ScratchProject {
  const archive = unzipSync(new Uint8Array(contents));
  return JSON.parse(strFromU8(archive['project.json'] as Uint8Array)) as ScratchProject;
}

interface PalettePngInspection {
  colorType?: number;
  height?: number;
...
function inspectPalettePng(contents: Buffer): PalettePngInspection {
  assert.deepEqual(
  ...
  for (let offset = 8; offset + 12 <= contents.length; ) {
    ...
    else if (type === 'tRNS') transparent = data.some((alpha: number) => alpha < 255);
    offset += length + 12;
    if (type === 'IEND') break;
  }
...
function assertLoadingBubbleAnchor(project: ScratchProject, description: string): void {
...
function assertPromptPosition(project: ScratchProject, description: string): void {
...
function assertLoadingSkinPosition(project: ScratchProject, description: string): void {
  const loading = project.targets.find((target) => target.name === 'Loading');
  assert.ok(loading, `${description}: Loading target is missing`);
  assert.deepEqual(
    [loading.x, loading.y],
...
test('keeps the migrated Scratch assets complete and content-addressed', async () => {
  const directories: [directory: string, expectedCount: number][] = [
    ['images', 26],
    ['sounds', 22],
  ];
...
test('keeps all Urashima actor costumes as compact transparent 64-color PNGs', async () => {
  interface ActorCostume {
    ...
    contentType: string;
    dataFormat: string;
    metadata: {bitmapResolution: number; rotationCenterX: number; rotationCenterY: number};
  }
  const manifest = JSON.parse(
    await readFile(path.join(sampleDirectory, 'assets.lock.json'), 'utf8'),
  ) as {assets: ActorCostume[]};
  const actorAssets = manifest.assets.filter(
    (asset) => asset.kind === 'costume' && asset.target === 'Actor',
  );
  const expectedGeometry: Record<string, [number, number, number, number]> = {
  ...
  for (const asset of actorAssets) {
    ...
    assert((png.paletteEntries ?? 0) <= 64, `${asset.name} exceeds 64 palette entries`);
    assert.equal(png.transparent, true, `${asset.name} lost transparency`);
    const geometry = expectedGeometry[asset.name];
    assert.ok(geometry, `${asset.name} has no expected geometry`);
    const [width, height, rotationCenterX, rotationCenterY] = geometry;
  ...
  const compactBytes = actorAssets.reduce((total: number, asset) => total + asset.size, 0);
...
test('pins the generic, editor, and player profile contract', async () => {
  ...
  const baseProject = JSON.parse(
    strFromU8(baseArchive['project.json'] as Uint8Array),
  ) as ScratchProject;
  assert(baseProject.extensions?.includes('tmposebundle'));
  const tmposeBundleUrl = baseProject.extensionURLs?.tmposebundle;
  assert.ok(tmposeBundleUrl, 'generic base: tmposebundle extension URL is missing');
  const tmposeBundleSource = Buffer.from(
    tmposeBundleUrl.split(',')[1] as string,
    'base64',
  ).toString('utf8');
  ...
  assert.ok(baseStage, 'generic base: stage target is missing');
  ...
  const finalBrightnessByTransition = Object.fromEntries(
    ['fadeOut', 'fadeUp', 'fadeToWhite', 'fadeFromWhite'].map((transitionAction) => {
      ...
      const blockAt = (id: string | null | undefined) => {
        assert.ok(id, `${transitionAction}: block reference is missing`);
        const block = baseStage.blocks[id];
        assert.ok(block, `${transitionAction}: block ${id} is missing`);
        return block;
      };
      const definition = blockAt(prototypeEntry[1].parent);
      const repeat = blockAt(definition.next);
      const finalBrightness = blockAt(repeat.next);
  ...
  assert.ok(stage, 'Urashima base: stage target is missing');
  ...
  assert.match(
    strFromU8(titleArchive[titleCostume.md5ext as string] as Uint8Array),
    /Version 3\.2\.2 \(2026\/08\/05\)/,
    'Urashima base: Title has an unexpected fallback version label',
  );
  assert.doesNotMatch(
    strFromU8(titleArchive[runtimeTitleCostume.md5ext as string] as Uint8Array),
    /<text\b/u,
...
test('keeps my-urashima external-script-only and publishes its DSL 4.0 workshop source', async () => {
  ...
  assert.ok(stage, 'my-urashima: stage target is missing');
  assert.ok(actor, 'my-urashima: Actor target is missing');
  ...
  assert.ok(princessCostume, 'my-urashima: Princess costume is missing');
  ...
  assert.equal(
    Buffer.from(archive[princessCostume.md5ext as string] as Uint8Array).equals(princessSource),
    true,
  );
  assert.equal(
  ...
  const dsl4Document = parseYaml(dsl4) as Dsl4Story & Record<string, any>;
...
test('keeps shared Packager output disabled unless a sample enables it', async () => {
  assert.deepEqual(
    await buildPackagedWeb({inputSb3Path: '', outputSampleDirectory: ''}),
    {enabled: false},
  );
});

test('recovers interrupted or stalled-running WebKit audio after touch completion', async () => {
  // 音声アンロックスクリプトを VM サンドボックスで動かすための最小限のブラウザ模造。
  type SandboxListener = (event: Record<string, unknown>) => void;
  const createEventTarget = () => {
    const listeners = new Map<string, Set<SandboxListener>>();
    return {
      listeners,
      addEventListener(eventName: string, listener: SandboxListener) {
        if (!listeners.has(eventName)) listeners.set(eventName, new Set());
        listeners.get(eventName)!.add(listener);
      },
      removeEventListener(eventName: string, listener: SandboxListener) {
        listeners.get(eventName)?.delete(listener);
      },
      dispatch(eventName: string, properties: Record<string, unknown> = {}) {
  ...
  const runNextTimer = () => {
    ...
    assert.ok(timer, 'a pending audio-unlock timer is required');
    timer.callback();
  };
  const audioContext = {
    state: 'interrupted',
  ...
  }) as unknown as ReturnType<typeof createEventTarget> & {
...
test('locks every external script asset and publishes DSL 3.2 and offline DSL 4.0 scripts', async () => {
  ...
  const assetManifest = validateAssetManifest(rawAssetManifest) as {
    assets: {name: string; kind: string; contentType: string; dataFormat: string}[];
  };
  ...
  assert(dsl4.startsWith('kamishibai: "4.0"\n'));
  const dsl4Document = parseYaml(dsl4) as Dsl4Story & Record<string, any>;
  assert.ok(dsl4Document.assets, 'urashima.k4.yml must declare assets');
