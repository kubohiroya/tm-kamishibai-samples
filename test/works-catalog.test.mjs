...
} from "../scripts/works-catalog.mjs";
...
test("publishes the approved works-library categories and rights metadata", async () => {
  const catalog = await readWorksCatalog(
    path.join(projectRoot, "site/works.json"),
  );
  ...
  assert(
    urashima.actions.some(
      ({ href, download }) =>
        href === "stories/urashima/urashima-4.0.sb3" && download,
    ),
  );
  assert(
  ...
  assert.equal(
    urashima.actions.find(({ href }) => href === "stories/urashima/web-4.0/")
      .requires,
    "urashimaDsl4Web",
  );
  assert.equal(
    urashima.actions.some(({ group }) => group === "作品情報"),
    false,
  );
  ...
  assert.equal(
    myUrashima.actions.find(
      ({ href }) => href === "stories/my-urashima/web-4.0/",
    ).requires,
    "myUrashimaDsl4Web",
  );
  assert.equal(
    myUrashima.actions.some(({ group }) => group === "作品情報"),
    false,
  );
  assert.deepEqual(
    myUrashima.actions
  ...
  delete partiallyGrouped.works.find(({ id }) => id === "urashima").actions[0]
    .group;
  assert.throws(
...
test("allows external works only as one HTTPS link with rights and terms", () => {
  ...
  assert.throws(
    () => validateWorksCatalog(downloadable),
    /must not offer a download/u,
  );
