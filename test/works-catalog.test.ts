...
import {readWorksCatalog, validateWorksCatalog, type WorkEntry} from '../scripts/works-catalog.ts';
import {verifyExternalWorkLinks, type FetchLike} from '../scripts/verify-external-works.ts';
...
test('publishes the approved works-library categories and rights metadata', async () => {
  ...
  assert.ok(urashima?.thumbnail, 'urashima must exist with a thumbnail');
  ...
  assert.ok(myUrashima?.thumbnail, 'my-urashima must exist with a thumbnail');
  ...
  const partiallyGroupedUrashima = partiallyGrouped.works.find(({id}) => id === 'urashima');
  assert.ok(partiallyGroupedUrashima);
  delete partiallyGroupedUrashima.actions[0]?.group;
  assert.throws(
  ...
  const absoluteUrashima = absoluteDetailHref.works.find(({id}) => id === 'urashima');
  assert.ok(absoluteUrashima);
  absoluteUrashima.detailHref = 'https://example.com/urashima';
  assert.throws(
...
test('allows external works only as one HTTPS link with rights and terms', () => {
  ...
  const downloadable = structuredClone(catalog) as unknown as {
    works: {actions: {download?: boolean}[]}[];
  };
...
test('detects unavailable external work and terms links', async () => {
  ...
  // 検証に使うのは ok と status だけなので、Response 全体は模造しない。
  ...
  const fetchBroken = (async (url: string) => ({
    ok: !url.endsWith('/work'),
    status: 404,
  })) as unknown as FetchLike;
