...
公開は`tmpose-kamishibai` `4.0.0-rc.8`のrelease固定だけに依存します。samplesを先に公開し、
...
完成版とaddition kitは`poseRecognition.preview.overlay`を使い、手首ジョイントとボーンをカメラ映像へ重ねて表示します。これは`4.0.0-rc.8`が依存する`@kubohiroya/turbowarp-tmpose@1.12.0`のoverlay APIを利用する例です。
