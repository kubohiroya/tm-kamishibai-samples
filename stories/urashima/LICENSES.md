...
`urashima-4.0.sb3`は、上記コンテンツと3ポーズモデルに加え、`tmpose-kamishibai` PR #519のcommit `160bdfae48bf4bb8c8bbebbbb72fd7582d114853`から生成したDSL 4.0 runtime baseを含みます。統合されたJavaScriptの冒頭には、各機能拡張ソースの名称、著作権表示、ライセンス表示を保持しています。ポーズモデルの取得元と各ファイルの固定SHA-256は`dsl4-inputs.lock.json`に記録しています。
