...
`urashima-4.0.sb3`は、上記コンテンツと3ポーズモデルに加え、`tmpose-kamishibai` PR #519のcommit `41dcf1d6051b11cf28148db85dd6377ec6f9fb95`から生成したDSL 4.0 runtime baseを含みます。統合されたJavaScriptの冒頭には、各機能拡張ソースの名称、著作権表示、ライセンス表示を保持しています。ポーズモデルの取得元と各ファイルの固定SHA-256は`dsl4-inputs.lock.json`に記録しています。
