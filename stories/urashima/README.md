...
- `base/kamishibai-4.0.sb3`: DSL 4.0安定版ソースcommitから生成した固定`generic`ベース
...
`dsl4-build.config.json`に固定した`tmpose-kamishibai` commitを、既定では隣接する`../tmpose-kamishibai`にcheckoutして依存関係をインストールします。CIでは同じcommitを`.tmp/tmpose-kamishibai`へcheckoutし、`TMPOSE_KAMISHIBAI_DSL4_ROOT`でその場所を指定します。入力となる`base/kamishibai-4.0.sb3`も、DSL 4.0安定版ソースcommit、サイズ、SHA-256を設定に固定しています。
...
- DSL 4.0汎用ベースのソース: `tmpose-kamishibai` / `98cec94387cac726e947cf2454e3f0c5e9146061`
- DSL 4.0ビルダー: `tmpose-kamishibai` / `8ea06bfd100b106f559cb25a280fab5570e42919`
