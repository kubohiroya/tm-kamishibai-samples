...
浦島太郎の元台本、画像・音声、アセットロック、生成設定を管理します。公開用SB3は、固定したnpmビルダー `3.1.7` と、同じリリースの汎用ベースからGitHub Pagesのビルド時に生成します。
...
冒頭のテキストは `action=text:Narration:...` と `action=wait:...` を交互に記述し、シーン内の時系列に沿って更新します。汎用ベースは、固定英語フォールバックと実行時生成によるTitle画面の多言語表示に対応した本体3.1.7コミット `91ca1dadbfdd037e6f0e0f45e80941c777ab242e` から生成しています。
...
- `base/kamishibai.sb3`: `tmpose-kamishibai` `v3.1.7` / `91ca1dadbfdd037e6f0e0f45e80941c777ab242e` の `generic` 成果物
...
scene 7では、本体3.1.7の標準コマンド`fadeToWhite`でステージの明るさを`+100`へ上げたまま保持してから背景をSmokeへ切り替え、`fadeFromWhite`で`0`へ戻して煙を見せます。通常の`fadeOut`（`-100`で保持）と`fadeUp`（`0`へ復帰）は変更しません。Asset Managerのクローン互換処理だけを生成時に適用し、パッチ後の一時ベースも`sample.config.json`のサイズとSHA-256で検証します。
...
- 汎用ベース: `tmpose-kamishibai` `v3.1.7` / `91ca1dadbfdd037e6f0e0f45e80941c777ab242e`
