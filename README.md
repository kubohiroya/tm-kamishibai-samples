...
- `base/kamishibai-4.0.sb3`: DSL 4.0.0-rc.1の固定release sourceから生成し、サイズとSHA-256を固定した`generic`ベース
...
`stories/tutorial/`にはDSL 4.0チュートリアル用の3場面作品、starter、addition kit、自己完結SB3、Web版の固定情報があります。`tmpose-kamishibai#548`で固定・公開した`4.0.0-rc.1`を入力として、`publication.enabled`をONにしたPagesと作品一覧を先に公開します。docs側はその公開URLとintegrityを使ってcaptureとチュートリアルpublicationを行います。生成規約とロールバック方法は[`stories/tutorial/README.md`](stories/tutorial/README.md)に記載しています。
...
- `tutorial-4.0.sb3` / starter ZIP / addition kit ZIP / `web-4.0/index.html`: 4.0.0-rc.1で再現・検証して公開するDSL 4.0チュートリアル成果物
