...
async function verifyFile(outputSampleDirectory, record, description) {
  const contents = await readFile(
    path.join(outputSampleDirectory, record.path),
  );
...
async function verifyProfile(outputSampleDirectory, profile, record) {
  ...
  assert.equal(
    record.scriptMode,
    profile === "player" ? "embedded" : "external",
  );
  const [sb3Contents, scriptContents, builderManifestContents] =
    await Promise.all([
  ...
  assert.equal(
    builderManifest.builder.package,
    "@kubohiroya/tmpose-kamishibai",
  );
  assert.equal(builderManifest.builder.version, "3.2.2");
  validateBundle({
    sb3Bytes: sb3Contents,
    scriptBytes: scriptContents,
    manifest: builderManifest,
  });
  assert.equal(
    /^(?:asset=.*,(?:file|https?):)/mu.test(scriptContents.toString("utf8")),
    false,
  );
...
function verifySiteHeader(html, assetPrefix) {
  ...
  assert(
    html.includes('<a class="skip-link" href="#main-content">本文へ移動</a>'),
  );
  assert(html.includes('<header class="site-header">'));
  assert(
    html.includes(
      `<link rel="stylesheet" href="${assetPrefix}site-shell.css">`,
    ),
  );
  assert(
    html.includes(
  ...
  assert(
    html.includes(
      `data-navigation-contract-version="${NAVIGATION_CONTRACT.contractVersion}"`,
    ),
  );
  for (const destination of destinations) {
    assert(
      html.includes(`href="${destination}"`),
      `Missing site destination: ${destination}`,
    );
  }
...
function verifySiteFooter(html, assetPrefix) {
  assert.equal(
    (
      html.match(
        /<footer class="site-footer" data-site-footer-version="1">/gu,
      ) ?? []
    ).length,
    1,
  );
  const footer =
    html.match(/<footer class="site-footer"[\s\S]*?<\/footer>/u)?.[0] ?? "";
...
export async function verifyPublishedSite(options = {}) {
  ...
  const outputDirectory =
    options.outputDirectory ?? path.join(projectRoot, "dist");
  const sourceDirectory =
    options.sourceDirectory ?? path.join(projectRoot, "stories/urashima");
  const outputSampleDirectory = path.join(outputDirectory, "stories/urashima");
  const myUrashimaSourceDirectory = path.join(
    projectRoot,
    "stories/my-urashima",
  );
  const myUrashimaOutputDirectory = path.join(
    outputDirectory,
    "stories/my-urashima",
  );
  ...
  const [
    ...
    readFile(path.join(outputSampleDirectory, "manifest.json"), "utf8").then(
      JSON.parse,
    ),
    readFile(
      path.join(myUrashimaOutputDirectory, "dsl4-manifest.json"),
      "utf8",
    ).then(JSON.parse),
    readFile(path.join(sourceDirectory, "dsl4-build.config.json"), "utf8").then(
      JSON.parse,
    ),
    readFile(
      path.join(myUrashimaSourceDirectory, "dsl4-build.config.json"),
      "utf8",
    ).then(JSON.parse),
    ...
    readFile(path.join(outputDirectory, "works.schema.json"), "utf8").then(
      JSON.parse,
    ),
  ...
  for (const work of worksCatalog.works.filter(
    ({ distribution }) => distribution === "hosted",
  )) {
    assert(
      work.thumbnail,
      `${work.id} is missing its representative scene image.`,
    );
    const thumbnail = await stat(
      path.join(outputDirectory, work.thumbnail.src),
    );
    assert(thumbnail.isFile(), `${work.id} thumbnail is not a file.`);
    assert(thumbnail.size > 0, `${work.id} thumbnail is empty.`);
    assert(thumbnail.size <= 100_000, `${work.id} thumbnail exceeds 100 KB.`);
  }
  assert.equal(
    worksSchema.$id,
  ...
  for (const relativePath of sourceFiles) {
    ...
    assert(
      ...
      `Published file differs from source: ${relativePath}`,
    );
  }
  ...
  assert(
    myUrashimaDsl4Published.toString("utf8").startsWith('kamishibai: "4.0"\n'),
  );
  ...
  assert.deepEqual(manifest.assetCounts, {
    images: 26,
    sounds: 22,
    embedded: 44,
  });
  ...
  const [
    ...
    verifyFile(
      outputSampleDirectory,
      manifest.source.dsl4Script,
      "DSL 4.0 script",
    ),
    verifyFile(
      outputSampleDirectory,
      manifest.source.assetManifest,
      "asset manifest",
    ),
  ]);
  ...
  assert.equal(
    player.builderManifest.script.embeddedVariableId,
    "tmposeEmbeddedScript",
  );
  ...
  assert(
    manifest.web.packager.projectExtensions.some(
      ({ id }) => id === "tmposebundle",
    ),
  ...
  const webHtml = await verifyFile(
    outputSampleDirectory,
    manifest.web.output,
    "Packager HTML",
  );
  ...
  assert(
    webHtml.includes(Buffer.from("<title>浦島太郎 | TMPose紙芝居</title>")),
  );
  ...
  if (dsl4WebEnabled) {
    ...
    assert.deepEqual(manifest.dsl4Web.reproducibility, {
      runs: 2,
      identical: true,
    });
    ...
    assert(
      dsl4WebHtml.includes(
        Buffer.from("<title>浦島太郎 DSL 4.0 | TMPose紙芝居</title>"),
      ),
    );
  } else {
  ...
  if (myDsl4WebEnabled) {
    ...
    assert.deepEqual(
      myDsl4Manifest.web.runtimeCapabilities.map(
        ({ capability }) => capability,
      ),
      ["file-picker", "camera", "audio"],
    );
    assert.deepEqual(myDsl4Manifest.web.reproducibility, {
      runs: 2,
      identical: true,
    });
  ...
  assert(
    !rootIndex.includes(
      "https://kubohiroya.github.io/tmpose-kamishibai/stories/",
    ),
  );
  ...
  assert.equal(
    dsl4WebEnabled
      ...
      : sampleIndex.includes("Web版（準備中）") &&
          !sampleIndex.includes('href="web-4.0/"'),
    true,
  );
  ...
  assert(urashimaCard.includes('src="stories/urashima/card-scene.webp"'));
  assert(urashimaCard.includes('loading="lazy" decoding="async"'));
  assert(
    urashimaCard.includes("代表シーン画像：Hiroya Kuboおよび個別素材の権利者"),
  );
  ...
  assert(myUrashimaCard.includes('src="stories/my-urashima/card-scene.webp"'));
  assert(rootIndex.includes('src="stories/tutorial/card-scene.webp"'));
  assert.equal(
    (rootIndex.match(/loading="lazy" decoding="async"/gu) ?? []).length,
    3,
  );
  ...
  assert(
    !myUrashimaDsl40Actions.includes(
      'href="stories/my-urashima/my-urashima.sb3"',
    ),
  );
  assert.equal(
    myDsl4WebEnabled
      ...
      : myUrashimaDsl40Actions.includes(
          ...
          !myUrashimaDsl40Actions.includes(
            'href="stories/my-urashima/web-4.0/"',
          ),
    true,
  );
  assert(
  ...
  assert.equal(
    rootIndex.includes("準備中"),
    !dsl4WebEnabled || !myDsl4WebEnabled,
  );
  ...
  assert(
    rootIndex.indexOf(">浦島太郎</a></h3>") <
      rootIndex.indexOf(
        ">my-urashima（ワークショップにおける作業用）</a></h3>",
      ),
  );
  assert(
  ...
  assert(
    sampleDsl32Actions.includes('<h2 id="dsl-32-heading">DSL 3.2 実行版</h2>'),
  );
  ...
  assert(publishedFiles.includes("stories/my-urashima/card-scene.webp"));
  ...
  assert(publishedFiles.includes("stories/urashima/card-scene.webp"));
  assert(publishedFiles.includes("stories/tutorial/card-scene.webp"));
  assert.equal(
    publishedFiles.includes("stories/urashima/web-4.0/index.html"),
    dsl4WebEnabled,
  );
  assert(
  ...
  assert(
    !publishedFiles.includes(
      "stories/urashima/licenses/tmpose-kamishibai-MIT.txt",
    ),
  );
  await verifyMyUrashimaOutput(
    path.join(outputDirectory, "stories/my-urashima"),
  );

  const [siteCss, faviconMetadata, favicon] = await Promise.all([
    readFile(path.join(outputDirectory, "site-shell.css"), "utf8"),
    readFile(path.join(outputDirectory, "favicon.source.json"), "utf8").then(
      JSON.parse,
    ),
  ...
  assert.match(
    siteCss,
    /\.site-footer__rights\s*\{[\s\S]*?min-height:\s*44px;/u,
  );
  assert.match(
    siteCss,
    /@media print[\s\S]*?\.site-footer\s*\{[\s\S]*?display:\s*none/u,
  );
...
if (
  ...
  console.log(
    `Verified ${results.fileCount} published files and ${results.assetCount} assets.`,
  );
}
