...
import {verifyMyUrashimaOutput} from './build-my-urashima.ts';
import {NAVIGATION_CONTRACT, renderSiteNavigation} from './site-navigation.ts';
import type {
  FileRecord,
...
import {validateWorksCatalog, type WorksCatalog} from './works-catalog.ts';

export interface VerifyPublishedSiteOptions {
  projectRoot?: string;
  outputDirectory?: string;
  sourceDirectory?: string;
}

/** ビルダーが SB3 と一緒に書き出す manifest。ここで参照する部分だけを型にする。 */
interface BuilderManifestFile {
  profile: string;
  outputName: string;
  script: {mode: string; embeddedVariableId: string | null; [key: string]: unknown};
  assets: unknown[];
  builder: {package: string; version: string; [key: string]: unknown};
  [key: string]: unknown;
}

interface ProfileVerification {
  builderManifest: BuilderManifestFile;
  sb3Contents: Buffer;
  scriptContents: Buffer;
}
...
async function verifyLinks(htmlPath: string): Promise<string> {
...
async function verifyProfile(
  ...
  const builderManifest = JSON.parse(
    builderManifestContents.toString('utf8'),
  ) as BuilderManifestFile;
...
function verifySiteHeader(html: string, assetPrefix: string): void {
...
function verifySiteFooter(html: string, assetPrefix: string): void {
  assert.equal(
...
export async function verifyPublishedSite(options: VerifyPublishedSiteOptions = {}) {
  ...
  const [
    ...
    await Promise.all([
      ...
      readFile(path.join(outputSampleDirectory, 'manifest.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as SiteManifest,
      ),
      readFile(path.join(myUrashimaOutputDirectory, 'dsl4-manifest.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as MyUrashimaDsl4Manifest,
      ),
      readFile(path.join(sourceDirectory, 'dsl4-build.config.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as Dsl4BuildConfig,
      ),
      readFile(path.join(myUrashimaSourceDirectory, 'dsl4-build.config.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as Dsl4BuildConfig,
      ),
      readFile(path.join(projectRoot, 'package.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as {devDependencies: Record<string, string>},
      ),
      ...
      readFile(path.join(outputDirectory, 'works.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as WorksCatalog,
      ),
      readFile(path.join(outputDirectory, 'works.schema.json'), 'utf8').then(
        (contents) => JSON.parse(contents) as {$id: string},
      ),
  ...
  assert.equal(
    ...
    '65f5e705921b6c92ba6ec5373ec13eff5101f2c6',
  );
  ...
  assert(manifest.web.enabled, 'The published Urashima manifest must enable its Web build.');
