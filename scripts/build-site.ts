...
import {buildMyUrashima} from './build-my-urashima.ts';
import {buildMyUrashimaDsl4} from './build-my-urashima-dsl4.ts';
import {buildUrashima} from './build-urashima.ts';
import {buildUrashimaDsl4} from './build-urashima-dsl4.ts';
import {buildPackagedWeb} from './build-packaged-web.ts';
import {
  buildTutorialDsl4,
  createTutorialPublicSurfaces,
  type TutorialPublicSurfaces,
} from './build-tutorial-dsl4.ts';
import {renderSiteHeader as renderContractSiteHeader} from './site-navigation.ts';
import {refreshChangedStoryArtifacts} from './refresh-story-artifacts.ts';
import {verifyTutorialCandidate} from './verify-tutorial-dsl4.ts';
import {verifyPublishedSite} from './verify-site.ts';
import {
  readWorksCatalog,
  ...
  type WorkEntry,
  type WorksCatalog,
} from './works-catalog.ts';
import type {Sb3BundleResult} from '@kubohiroya/tmpose-kamishibai/builder';
import type {
  ArtifactSizeEntry,
  AssetRecord,
  FileRecord,
...
import type {ArtifactsLock, ProfileLock} from './story-config.ts';
...
function renderArtifactSizes(label: string, artifacts: readonly ArtifactSizeEntry[]): string {
...
function renderSiteHeader(assetPrefix: string, pathname = '/tm-kamishibai-samples/'): string {
...
function renderSiteFooter(assetPrefix: string): string {
...
function detailRelativeHref(work: WorkEntry, href: string): string {
...
function renderDetailMediaGallery(work: WorkEntry): string {
...
async function fileRecord(filePath: string, publishedPath: string): Promise<FileRecord> {
  const contents = await readFile(filePath);
  return {path: publishedPath, size: contents.length, sha256: sha256(contents)};
}

async function assetRecords(directory: string, kind: string): Promise<AssetRecord[]> {
...
function renderDisabledAction(className: string, label: string): string {
  return `        <button class="${className}" type="button" disabled aria-disabled="true">${escapeHtml(label)}</button>`;
}

function renderWorkAction(action: WorkAction, features: SiteFeatures): string {
...
function renderWorkActions(work: WorkEntry, features: SiteFeatures): string {
...
function renderWorkCard(work: WorkEntry, features: SiteFeatures): string {
...
export function renderRootIndex(
  ...
  myDsl4Manifest: MyUrashimaDsl4Manifest,
...
export function renderMyUrashimaIndex(
  work: WorkEntry,
  dsl4Manifest: MyUrashimaDsl4Manifest,
  dsl32Artifacts: ArtifactsLock,
): string {
  ...
  const webHash = dsl4Manifest.web.enabled
    ...
    : '';
  const dsl32Output = dsl32Artifacts.output as Record<string, {size: number}> | undefined;
  const dsl32Sizes = renderArtifactSizes('DSL 3.2成果物のファイルサイズ', [
    {name: '作業用SB3ファイル', record: dsl32Output?.sb3},
    {name: '台本ファイル', record: dsl32Output?.script},
  ]);
  const dsl4Sizes = renderArtifactSizes('DSL 4.0成果物のファイルサイズ', [
    {
      name: 'Web版ファイル',
    ...
    {name: '台本ファイル（YAML）', record: dsl4Manifest.source as {size: number}},
...
export function renderTutorialIndex(
  work: WorkEntry,
  manifest: TutorialPublicSurfaces,
): string {
...
function profileRecord(
  profile: string,
  build: Sb3BundleResult,
  lock: {manifest: {sha256: string}},
): ProfileRecord {
...
export function verifyConfiguredWebBuild(
  web: {enabled: boolean},
  webConfig: {enabled?: unknown} | undefined,
  artifactLock: unknown,
  description: string,
): void {
...
export async function buildSite() {
  ...
  // verifyArtifacts の既定値が true なので、ロックは必ず読み込まれている。
  assert(artifactsLock?.storyDate, 'The Urashima artifact lock must record a story date.');
  assert(artifactsLock.profiles, 'The Urashima artifact lock must record profile digests.');
  assert(artifactsLock.web, 'The Urashima artifact lock must record its Web artifacts.');
  const myUrashimaBuild = await buildMyUrashima(myOutputSampleDirectory);
  assert(
    myUrashimaBuild.artifactLock.storyDate,
    'The my-urashima artifact lock must record a story date.',
  );
  assert.equal(
  ...
  const embeddedPaths = new Set(
    assetManifest.assets.map((asset: {uri: string}) => asset.uri.replace(/^file:/u, '')),
  );
  ...
  const manifest = {
    ...
    profiles: {
      editor: profileRecord('editor', editor, artifactsLock.profiles.editor as ProfileLock),
      player: profileRecord('player', player, artifactsLock.profiles.player as ProfileLock),
    },
    dsl4Offline: {
