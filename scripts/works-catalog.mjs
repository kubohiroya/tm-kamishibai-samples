...
function validateAction(action, label) {
  ...
  assert(
    ["primary", "secondary"].includes(action.style),
    `${label}.style is invalid.`,
  );
  if (Object.hasOwn(action, "group"))
    assertString(action.group, `${label}.group`);
  if (Object.hasOwn(action, "disabled")) {
    assert.equal(
      action.disabled,
      true,
      `${label}.disabled must be true when present.`,
    );
  }
  if (action.disabled) {
    for (const key of ["href", "download", "external", "requires"]) {
      assert(
        ...
        `${label}.${key} is not allowed when disabled.`,
      );
    }
  } else {
    assertString(action.href, `${label}.href`);
  }
  if (Object.hasOwn(action, "download"))
    assert.equal(typeof action.download, "boolean");
  if (Object.hasOwn(action, "external"))
    assert.equal(typeof action.external, "boolean");
  if (Object.hasOwn(action, "requires")) {
    assert(
      ["urashimaWeb", "urashimaDsl4Web", "myUrashimaDsl4Web"].includes(
        action.requires,
      ),
...
function validateThumbnail(thumbnail, label) {
  ...
  assert.equal(
    typeof thumbnail.alt,
    "string",
    `${label}.alt must be a string.`,
  );
...
export function validateWorksCatalog(catalog) {
  ...
  for (const [index, work] of catalog.works.entries()) {
    ...
    workIds.add(work.id);
    assert(
      categoryOrder.includes(work.category),
    ...
    assert(
      ["hosted", "link-only"].includes(work.distribution),
    ...
    assert(
      Array.isArray(work.actions) && work.actions.length > 0,
      `${label}.actions is empty.`,
    );
    work.actions.forEach((action, actionIndex) =>
      validateAction(action, `${label}.actions[${actionIndex}]`),
    );
    const groupedActionCount = work.actions.filter(
      (action) => action.group,
    ).length;
    assert(
    ...
    if (work.category === "external") {
      assert.equal(
        work.distribution,
        "link-only",
        `${label} must be link-only.`,
      );
      assert(
        work.license.href.startsWith("https://"),
        `${label}.license.href must use HTTPS.`,
      );
      if (work.thumbnail) {
        assert(
          work.thumbnail.src.startsWith("https://"),
          `${label}.thumbnail.src must use HTTPS.`,
        );
        assert(
      ...
      assert.equal(
        work.actions.length,
        1,
        `${label} must expose exactly one external action.`,
      );
      const [action] = work.actions;
      assert(!action.disabled, `${label} external action must be enabled.`);
      assert(
        action.href.startsWith("https://"),
        `${label} action must use HTTPS.`,
      );
      assert.equal(
        action.external,
        true,
        `${label} action must be marked external.`,
      );
      assert.notEqual(
        action.download,
        true,
        `${label} must not offer a download.`,
      );
      assert(
        !action.requires,
        `${label} must not depend on a local build feature.`,
      );
    } else {
      ...
      for (const action of work.actions) {
        ...
        assert(
          !action.external,
          `${label} must not mark a hosted action as external.`,
        );
        assert(
          !/^(?:[a-z]+:|\/|\.\.)/iu.test(action.href),
