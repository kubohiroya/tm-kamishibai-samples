// Generated from scripts/site-navigation.ts. Do not edit by hand.
...
const contract = JSON.parse(contractLocations
    .map((location) => {
    try {
...
export function resolveCurrentSection(site, pathname) {
    const normalizedPath = normalizePath(pathname);
    const matches = contract.currentSectionRules
        .filter((rule) => rule.site === site && normalizedPath.startsWith(normalizePath(rule.pathPrefix)))
        .sort((left, right) => right.pathPrefix.length - left.pathPrefix.length);
    return matches[0]?.current ?? null;
}
export function renderSiteNavigation({ site, pathname }) {
...
export function renderSiteHeader({ assetBase = '', site, pathname, }) {
...
export function replaceSiteNavigation(source, options) {
    const navigation = renderSiteNavigation(options);
    const navPattern = /<nav\b[^>]*\bclass=(['"])[^'"]*\bsite-nav\b[^'"]*\1[^>]*>[\s\S]*?<\/nav>/iu;
    const repositoryPattern = /<a\b[^>]*\bclass=(['"])[^'"]*\bsite-repository\b[^'"]*\1[^>]*>[\s\S]*?<\/a>/iu;
    if (!navPattern.test(source))
        throw new Error('Document does not contain a site navigation.');
    const withNavigation = source.replace(navPattern, navigation.match(/<nav[\s\S]*?<\/nav>/u)?.[0] ?? '');
    if (!repositoryPattern.test(withNavigation))
        throw new Error('Document does not contain a site repository link.');
    const withRepository = withNavigation.replace(repositoryPattern, navigation.match(/<a\b[^>]*\bclass=(['"])[^'"]*\bsite-repository\b[\s\S]*?<\/a>/iu)?.[0] ?? '');
