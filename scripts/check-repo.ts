...
import type { StoryConfig } from "./story-config.ts";

const projectRoot = fileURLToPath(new URL("../", import.meta.url));

/** repo-policy.json。リポジトリ全体の不変条件をここに集約している。 */
...
function assertNoHumanFacingLegacyText(relativePath: string, source: string): void {
...
function assertNoSpacedTurboWarpProductNames(relativePath: string, source: string): void {
...
async function assertLicense(): Promise<void> {
...
async function assertPackage(policy: RepoPolicy): Promise<void> {
...
async function assertStoryContracts(policy: RepoPolicy): Promise<void> {
  const sampleConfig = await readJson<StoryConfig>("stories/urashima/sample.config.json");
...
async function assertHumanFacingText(): Promise<void> {
  const files = [
    "README.md",
    "WORKS_POLICY.md",
...
async function assertPackContents(): Promise<void> {
  ...
  const packRecord = JSON.parse(packOutput) as {filename: string} | {filename: string}[];
  const record = Array.isArray(packRecord) ? (packRecord[0] as {filename: string}) : packRecord;
  ...
  assert(files.includes("package/scripts/check-repo.ts"));
  assert(!files.some((file) => file.startsWith("package/dist/")));
}

async function assertLockAlignment(policy: RepoPolicy): Promise<void> {
