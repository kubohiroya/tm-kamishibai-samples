...
} from './patch-packager-scratch-render.ts';
import type {OnlineDependency, RuntimeCapability, WebConfig} from './story-config.ts';

/** @turbowarp/packager は型定義を同梱しないため、使う API 面だけをここで宣言する。 */
interface PackagerOptions {
  ...
  custom: {js: string; [key: string]: unknown};
  [key: string]: unknown;
}
...
interface PackagerModule {
  Packager: new () => PackagerInstance;
  loadProject(input: Uint8Array): Promise<unknown>;
}

const require = createRequire(import.meta.url);
const Packager = require('@turbowarp/packager') as PackagerModule;
const installedPackager = require('@turbowarp/packager/package.json') as {version: string};

export interface PackagedWebInput {
  profile: string;
  path: string;
  size: number;
  sha256: string;
...
export interface PackagedWebOutput {
  path: string;
  contentType: string;
  size: number;
  sha256: string;
...
export interface PackagedWebResult {
  enabled: true;
  publicPath: string;
  input: PackagedWebInput;
  output: PackagedWebOutput;
  packager: {
    package: string;
    version: string;
    options: {
    ...
    projectExtensions: ProjectExtensionRecord[];
    scratchRender: {
      repository: string;
      commit: string;
      upstreamPullRequest: number;
      readbackContext: string;
    };
  };
  scriptMode: string;
  assets: string;
  audioUnlock: {
    ...
    verification: {
      strategy: string;
      clockCheckDelayMs: number;
      primesOutput: boolean;
    } | null;
  };
  allowedOnlineDependencies: OnlineDependency[];
  runtimeCapabilities: RuntimeCapability[];
  reproducibility: {runs: number; identical: boolean};
}

export interface DisabledWebResult {
  enabled: false;
}

export type PackagedWeb = PackagedWebResult | DisabledWebResult;

export interface BuildPackagedWebOptions {
  inputSb3Path: string;
  outputSampleDirectory: string;
  rawWebConfig?: WebConfig | undefined;
...
function projectExtensions(sb3Bytes: Uint8Array): {
  urls: string[];
  records: ProjectExtensionRecord[];
} {
  const archive = unzipSync(new Uint8Array(sb3Bytes));
  const project = JSON.parse(strFromU8(archive['project.json'] as Uint8Array)) as {
...
function verifyPackagerResult(result: PackagerResult, webConfig: WebConfig): void {
...
export async function buildPackagedWeb({
  ...
  expectedInput,
  expectedOutput,
}: BuildPackagedWebOptions): Promise<PackagedWeb> {
  // 既定値とマージした時点では必須項目が揃っていないが、enabled が真なら
  // 直後の assert 群が実行時に完全な WebConfig であることを検証する。
  const webConfig = {...DEFAULT_WEB_CONFIGURATION, ...rawWebConfig} as WebConfig;
