...
import {
  ...
  type TutorialArtifactLock,
} from './build-tutorial-dsl4.ts';
import type {ScratchProject} from './scratch.ts';
import type {DependencyRecord, Dsl4BuildConfig} from './story-config.ts';

const defaultProjectRoot = fileURLToPath(new URL('../', import.meta.url));

export interface VerifyTutorialCandidateOptions {
  projectRoot?: string;
  candidateOutputDirectory?: string;
  outputDirectory?: string;
}

export interface TutorialCandidateVerification {
  published: boolean;
...
/** public-surfaces.json は createTutorialPublicSurfaces と同じ形。 */
type PublicSurfaces = Record<string, unknown> & {
  published?: boolean;
  sourceIdentity?: string;
...
/** チュートリアル台本のうち、この検証が参照する部分。 */
interface TutorialStory {
...
export async function verifyTutorialCandidate(
  options: VerifyTutorialCandidateOptions = {},
): Promise<TutorialCandidateVerification> {
  ...
  const [config, artifactLock, webLock, publicSurfaces, finalSource, starterSource, addition] =
    await Promise.all([
      readJson<Dsl4BuildConfig>(path.join(storyDirectory, 'dsl4-build.config.json')),
      readJson<TutorialArtifactLock>(path.join(storyDirectory, 'dsl4-artifacts.lock.json')),
      readJson<PackagedWebResult>(path.join(storyDirectory, 'dsl4-web-artifacts.lock.json')),
      readJson<PublicSurfaces>(path.join(storyDirectory, 'public-surfaces.json')),
      readFile(path.join(storyDirectory, 'story.kamishibai.yaml'), 'utf8').then(
        (source) => parse(source, {uniqueKeys: true}) as TutorialStory,
      ),
      readFile(path.join(storyDirectory, 'starter.kamishibai.yaml'), 'utf8').then(
        (source) => parse(source, {uniqueKeys: true}) as TutorialStory,
      ),
  ...
  const publication = config.publication as {enabled: boolean; reason: string};
  assert.equal(config.status, publication.enabled ? 'published' : 'candidate');
  assert.equal(publicSurfaces.published, publication.enabled);
  assert.deepEqual(
    publicSurfaces,
  ...
  // 台本の中身は動的な YAML なので、参照する経路だけをここでほどく。
  const scene = (name: string): Record<string, unknown> =>
    finalSource.scenes[name] as Record<string, unknown>;
  const protectScene = scene('protect');
  const protectActions = protectScene.actions as Record<string, Record<string, unknown>>[];
  const instructionScene = scene('instruction') as unknown as Record<
    string,
    Record<string, {text: string}>
  >;
  const successScene = scene('success') as unknown as Record<
    string,
    Record<string, {text: string}>
  >;
  assert.equal(protectScene.poseModel, 'SafetyPose');
  assert.equal(
    instructionScene[1]?.['Student.say']?.text,
    '自分の身を守るため、丈夫な机の下に入り、両手で頭を守ろう！',
  );
  assert.equal(
    (protectActions[2]?.['Student.pose'] as {steps: {pose: string}[]}).steps[0]?.pose,
    '頭を守る',
  );
  assert.deepEqual(protectActions[1]?.['Student.show'], {
    skin: 'ProtectHead',
    x: 0,
    y: -60,
    scale: 65,
  ...
  assert.equal(
    successScene[2]?.['Student.say']?.text,
  ...
  const project = JSON.parse(
    strFromU8(unzipSync(new Uint8Array(sb3))['project.json'] as Uint8Array),
  ) as ScratchProject;
  assert.deepEqual(
