...
import type {SiteManifest} from './site-manifest.ts';
import type {OnlineDependency} from './story-config.ts';
...
try {
  ...
  const allowedPrefixes: string[] = manifest.web.allowedOnlineDependencies.map(
    (dependency: OnlineDependency) => dependency.urlPrefix,
  );
  const requests: string[] = [];
  const rejectedRequests: string[] = [];
  const assetRegistrationErrors: string[] = [];
  const legacyTextWarnings: string[] = [];
  ...
  await page.evaluate(() => {
    ...
    renderer.createSVGSkin = (svg: any, ...args: any[]) => {
    ...
    window.__tmposeLoadingBackdropTimer = window.setInterval(() => {
      ...
      const costume = stage
        .getCostumes()
        .find((candidate: any) => candidate.skinId === skinId)?.name;
  ...
  const loadingBackdropState = await page.evaluate(() => {
    ...
    return {
      displayedStarsWhileLoading: window.__tmposeLoadingBackdropSamples.some(
        ({costume, loadingVisible}: any) => costume === 'Stars' && loadingVisible,
      ),
      sampledCostumes: [
        ...new Set(
          window.__tmposeLoadingBackdropSamples
            .map(({costume}: any) => costume)
            .filter(Boolean),
        ),
      ],
    };
  ...
  const startedState = await page.evaluate(() => {
    ...
    const stage = runtime.getTargetForStage();
    const variables = runtime.ext_lmsTempVars2.runtimeVariables;
    const stageVariable = (name: string) =>
      Object.values<any>(stage.variables).find((variable) => variable.name === name)?.value;
    return {
  ...
  await page.waitForFunction(
    () => {
      ...
      return (
        narration?.visible
        && window.__tmposeSvgTextSkins?.some(({svg}: any) =>
  ...
  const narrationState = await page.evaluate(() => {
    ...
    const svg = window.__tmposeSvgTextSkins
      .findLast((entry: any) => entry.svg.includes('<title>むかし</title>'))?.svg;
    return {
  ...
  const uiAssetState = await page.evaluate(async () => {
    ...
    const displayedUiItem = (name: any) => uiItemTarget(name)?.visible === true;
    ...
    return {
      ...
      staleTitleTargets: window.__tmposeTitleUiTargetIds.filter((id: any) =>
  ...
  const audioState = await page.evaluate(async () => {
    ...
    const testSound = sounds.find((sound: any) => sound.name === 'Sewing Machine');
    ...
    return {
      ...
      mp3SoundCount: sounds.filter((sound: any) => sound.dataFormat === 'mp3').length,
  ...
  const danceState = await page.evaluate(async () => {
    ...
    for (let index = 0; index < 12; index += 1) {
      ...
      const skinId = drawable?._skin?._id;
      fishSkinNames.push(fish.getCostumes().find((costume: any) => costume.skinId === skinId)?.name);
  ...
  const whiteTransitionState = await page.evaluate(() => {
    ...
    return {
      ...
      maximumBrightness: Math.max(...samples.map((sample: any) => sample.brightness)),
      maximumSmokeBrightness: Math.max(
        ...samples
          .filter((sample: any) => sample.costume === 'Smoke')
          .map((sample: any) => sample.brightness),
      ),
    };
  });
  ...
  await page.waitForFunction(
    () => {
      const runtime = window.scaffolding?.vm?.runtime;
      const stage = runtime?.getTargetForStage();
      const endCostume = stage?.getCostumes().find((costume: any) => costume.name === 'End');
      const drawable = runtime?.renderer?._allDrawables?.[stage?.drawableID];
      const narration = runtime?.targets.find((target) => {
        const actorName = target.lookupVariableByNameAndType?.('actorName', '')?.value;
        return !target.isStage && actorName === 'Narration';
