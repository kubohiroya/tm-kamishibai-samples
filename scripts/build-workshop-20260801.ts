...
/** fflate の zipSync に渡すエントリ表。 */
type ZipEntries = Record<string, [Uint8Array, { level: 0 | 9; mtime: Date }]>;

export interface WorkshopArchiveSummary {
  entryCount: number;
  size: number;
}

export interface AssembleWorkshopDistributionOptions {
  licensePath: string;
...
async function assertFile(filename: string, description: string): Promise<void> {
  let information;
  try {
...
async function copyVisibleDirectory(
...
async function collectZipEntries(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const level: 0 | 9 = storedExtensions.has(
...
export async function verifyWorkshopArchive(
  zipPath: string,
): Promise<Readonly<WorkshopArchiveSummary>> {
...
export async function assembleWorkshopDistribution({
  ...
  siteOutputDirectory,
  zipPath,
}: AssembleWorkshopDistributionOptions) {
  await Promise.all([
